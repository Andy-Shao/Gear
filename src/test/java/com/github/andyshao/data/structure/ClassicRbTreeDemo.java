 ...
 * @param <V> value type
 */
final class ClassicRbTreeDemo<K extends Comparable<K> , V> implements RbTree<K , V> {
    private final Bitree<RbTreeNode<K , V>> biTree;
    private final Supplier<RbTreeNode<K , V>> nodeFactory;

    public ClassicRbTreeDemo() {
        this(Bitree.defaultBitTree() , RbTreeNode::defaultNode);
    }

    public ClassicRbTreeDemo(Bitree<RbTreeNode<K , V>> biTree , Supplier<RbTreeNode<K , V>> nodeFactory) {
        this.biTree = biTree;
