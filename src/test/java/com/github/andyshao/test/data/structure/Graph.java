...
public interface Graph<D> extends Cleanable {
    ...
    public class MyGraph<DATA> implements Graph<DATA> {
        ...
        public DATA graph_rem_vertex(final DATA data) {
            ...
            if (element.data().adjacent().set_is_member(data)) throw new GraphOperationException("Do not allow removal of the vertex if it is an adjecency list.");
            //Return if the vertex was not found.
    ...
    public static <DATA> Graph<DATA> DEFAULT_GRAPH(Comparator<DATA> comparator , Supplier<SingleLinked<AdjList<DATA>>> singleLinkedFactory) {
