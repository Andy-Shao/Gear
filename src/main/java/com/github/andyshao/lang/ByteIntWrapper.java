...
public class ByteIntWrapper implements IntWrapper<byte[]> {
    ...
    public BigInteger size(byte[] array) {
        return array.length - (array.length >> ByteIntWrapper.BASE) > 0 ? BigInteger.valueOf((array.length >> ByteIntWrapper.BASE) + 1) : BigInteger.valueOf(array.length >> ByteIntWrapper.BASE);
    }
